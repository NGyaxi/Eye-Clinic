...
function addPat(event, userData) {
    ...
    connection.query(query, [phone], (error, results, fields) => {
        ...
        if (results.length > 0) {
            ...
            const insertQuery = 'INSERT INTO patients (fulname, age, patients_type, date_birth, telephone, occupation, patients_id, organization, gender, email, res_address, postal_add, date_added, time) VALUES (?, ?, ?, ?, ?, ?, ?)';
